...
import { Check, ChevronDown, ChevronUp, Edit2, Image as ImageIcon, Search, X } from 'lucide-react'
...
export default function SublimationInventory({ allSublimationProducts, token, onProductUpdated }) {
  ...
  const [localProducts, setLocalProducts] = useState(allSublimationProducts)

  useEffect(() => {
    setLocalProducts(allSublimationProducts)
  }, [allSublimationProducts])
  ...
  const filteredProducts = useMemo(() => {
    ...
    return localProducts.filter((product) => {
      const productUpc = String(product.upc || '').toLowerCase()
      const productName = String(product.name || '').toLowerCase()
      const matchesUpc = normalizedUpc ? productUpc.includes(normalizedUpc) : true
      const matchesName = normalizedName ? productName.includes(normalizedName) : true
      return matchesUpc && matchesName
    })
  }, [hasActiveFilter, localProducts, nameFilter, upcFilter])
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900">Inventario de sublimacion</h2>
        ...
        <Card className="border border-slate-200 shadow-sm">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">
                {hasActiveFilter
                  ? `${filteredProducts.length} resultado(s) editable(s)`
            ...
            <table className="w-full min-w-[760px] border-collapse text-sm">
              ...
              <tbody>
                {!hasActiveFilter ? (
                  ...
                  <tr>
                    <td colSpan="7" className="border border-slate-300 px-3 py-8 text-center text-slate-500">
                      No hay productos de sublimacion que coincidan con los filtros.
                    </td>
                  </tr>
                ) : (
                        ...
                        {isEditing ? (
                          ...
                          <>
                            <td className="border border-slate-300 px-3 py-2 font-mono text-slate-800">
                              {product.upc || <span className="text-slate-400">-</span>}
                            </td>
                            <td className="border border-slate-300 px-3 py-2 text-slate-800">{product.name}</td>
